...
def main() -> int:
    ...
    with process.log_process(interval=0.5, log_level=logger.getEffectiveLevel()) as process_thread:
        for i in range(8):
            size = random.randint(128, 128 + 256) * 1024 ** 2
            # Add a message to report in the next process write.
            process_thread.add_message_to_queue(f'String of {size / 1024**2:,.3f} MB')
            s = ' ' * size
